  ...
  >
    ...
    <nav class="hidden lg:flex items-center gap-6 ml-8">
      ...
      <router-link
        to="/workbench"
        class="flex items-center gap-2 transition-colors"
        :class="isNavActive('/workbench') ? '' : 'hover:opacity-80'"
...
const growthMenu: NavLink[] = [
  ...
  { path: '/habits', label: '习惯打卡', icon: 'repeat' },
...
const activeDropdownKey = computed<string>(() => {
  ...
  if (['/tasks', '/task-list', '/check-in', '/habits', '/achievements', '/kb-titles'].includes(p)) return 'growth';
