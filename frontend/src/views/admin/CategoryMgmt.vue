<template>
  <div class="cat-mgmt animate-fade-in">
    <nav class="flex items-center gap-2 text-sm text-muted-foreground mb-6">
      ...
      <span class="text-foreground font-medium">分类管理</span>
    </nav>
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-muted-foreground text-sm mt-1">管理知识库目录与分类结构</p>
      </div>
    ...
    <!-- 搜索框 -->
    <div class="bg-card border border-[var(--kb-border)] rounded-xl px-6 py-4 mb-4">
      <div class="max-w-md">
    ...
    <!-- 统计条 -->
    <div class="flex items-center gap-6 mb-4 text-sm text-muted-foreground">
      <span><strong class="text-foreground">{{ topLevelCount }}</strong> 个顶级分类</span>
      <span>/</span>
      <span><strong class="text-foreground">{{ subLevelCount }}</strong> 个子分类</span>
      <span>/</span>
      <span>共 <strong class="text-foreground">{{ totalCount }}</strong> 个</span>
    </div>

    <!-- 树形列表 -->
    <div class="bg-card border border-[var(--kb-border)] rounded-xl overflow-hidden">
      <!-- 表头 -->
      <div class="px-6 py-3 border-b border-[var(--kb-border)]">
        <div class="grid grid-cols-12 gap-4 text-xs text-muted-foreground font-medium items-center">
      ...
      <div v-if="flatTreeNodes.length" class="divide-y divide-[#E2E6EC]/50">
        <div
          ...
          :key="node.cat.id"
          class="px-6 py-3 hover:bg-muted/80 transition-colors group relative"
          :class="{
        ...
        >
          ...
          <div class="grid grid-cols-12 gap-4 items-center relative z-[1]">
            ...
            <div class="col-span-5 flex items-center gap-2 min-w-0">
              ...
              <!-- 名称 -->
              <span class="font-medium text-foreground truncate">{{ node.cat.name }}</span>
              <!-- 拖拽提示 -->
              <span
                v-if="dropTargetId === node.cat.id && dropPosition === 'inside'"
            ...
            <!-- 文档数 -->
            <div class="col-span-2 text-center text-sm text-muted-foreground">
            ...
            <div class="col-span-3 flex items-center justify-center gap-1">
              ...
              <button
                class="p-1.5 text-muted-foreground hover:text-blue-500 hover:bg-blue-50 rounded transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--kb-ring)] focus-visible:ring-offset-2"
      ...
      <!-- 空状态 -->
      <div v-else class="py-16 text-center text-muted-foreground text-sm">
      ...
      <div v-if="flatTreeNodes.length" class="px-6 py-3 text-center border-t border-[var(--kb-border)]/50">
        <span class="text-sm text-muted-foreground">显示 {{ flatTreeNodes.length }} / {{ allFlatCats.length }} 个分类</span>
      </div>
    </div>
    ...
    >
      <div class="bg-card rounded-xl w-full max-w-md p-6 animate-scale-in max-h-[90vh] overflow-y-auto">
        ...
        <div class="space-y-4">
          <!-- 名称 -->
          <div>
            <label class="block text-sm font-medium text-foreground mb-1.5">分类名称</label>
          ...
          <div>
            <label class="block text-sm font-medium text-foreground mb-1.5">
              父分类
              <span class="text-xs text-muted-foreground font-normal ml-1">（不选则为顶级分类）</span>
            </label>
            <CategoryTreeSelect
          ...
          <!-- 排序 -->
          <div>
            <label class="block text-sm font-medium text-foreground mb-1.5">排序</label>
    ...
    <div
      ...
      @keydown.enter.self.prevent="($event.target as HTMLElement).click()"
    >
      <div class="bg-card rounded-xl w-full max-w-sm p-6 animate-scale-in">
        <h3 class="text-lg font-semibold text-foreground mb-2">移动分类</h3>
        <p class="text-sm text-muted-foreground mb-4">
          将「<span class="text-foreground font-medium">{{ movingCat?.name }}</span>」移动到：
        </p>
        <CategoryTreeSelect
