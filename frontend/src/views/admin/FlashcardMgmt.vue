<template>
    <div class="space-y-6 animate-fade-in">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold text-foreground">知识卡片管理</h1>
          <p class="text-muted-foreground text-sm mt-1">管理学习闪卡内容，支持批量导入导出</p>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card class="bg-gradient-to-br from-primary-50 to-primary-100 border-primary-200">
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-sm text-muted-foreground">总卡片数</p>
        ...
        <Card class="bg-gradient-to-br from-green-50 to-green-100 border-green-200">
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-sm text-muted-foreground">已掌握</p>
              <p class="text-xl font-bold text-foreground">{{ stats.mastered }}</p>
            </div>
          </div>
        </Card>
        <Card class="bg-gradient-to-br from-yellow-50 to-yellow-100 border-yellow-200">
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-sm text-muted-foreground">学习中</p>
              <p class="text-xl font-bold text-foreground">{{ stats.learning }}</p>
            </div>
          </div>
        </Card>
        <Card class="bg-gradient-to-br from-red-50 to-red-100 border-red-200">
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-sm text-muted-foreground">待复习</p>
              <p class="text-xl font-bold text-foreground">{{ stats.due }}</p>
            </div>
          </div>
        </Card>
      </div>
      ...
      <Card padding="none">
          ...
          <div class="flex flex-col sm:flex-row sm:items-center gap-4">
            ...
            <div class="flex items-center gap-3 flex-wrap">
              <select
                ...
                class="px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:border-primary-500 hover:border-primary-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--kb-ring)] focus-visible:ring-offset-2 transition-colors"
              >
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
          <div
            ...
            class="group border border-border rounded-xl p-4 hover:shadow-md transition-colors transition-shadow duration-200 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--kb-ring)] focus-visible:ring-offset-2"
          ...
          >
            <div class="flex items-start justify-between mb-3">
              ...
              <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button class="p-1 hover:bg-gray-100 rounded text-muted-foreground hover:text-primary-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--kb-ring)] focus-visible:ring-offset-2 transition-colors" @click.stop="openEdit(card)">
                  <Icon name="edit" :size="16" aria-hidden="true" />
                </button>
                <button class="p-1 hover:bg-gray-100 rounded text-muted-foreground hover:text-danger-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--kb-ring)] focus-visible:ring-offset-2 transition-colors" @click.stop="deleteCard(card)">
                  <Icon name="trash-2" :size="16" aria-hidden="true" />
                </button>
              </div>
            </div>
            <h3 class="font-medium text-foreground mb-2 line-clamp-2">{{ card.front }}</h3>
            <p class="text-sm text-muted-foreground line-clamp-2 mb-3">{{ card.back }}</p>
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2 flex-wrap">
                <Badge v-for="tag in card.tags" :key="tag" variant="default" class="text-xs">{{ tag }}</Badge>
              </div>
              <span class="text-xs text-muted-foreground">{{ card.category }}</span>
            </div>
          ...
          <p v-if="filteredCards.length === 0" class="col-span-full text-center text-muted-foreground text-sm py-12">暂无闪卡数据</p>
        </div>
      </Card>
    </div>
    ...
    >
      <div class="bg-card rounded-xl w-full max-w-2xl shadow-xl animate-dropdown">
        <div class="flex items-center justify-between px-6 py-4 border-b border-border">
          <h3 class="text-lg font-semibold text-foreground">{{ editingId ? '编辑卡片' : '新增卡片' }}</h3>
        ...
        <div class="px-6 py-4 space-y-4">
          <div>
            <label class="block text-sm font-medium text-foreground mb-1.5">正面（问题）</label>
            <textarea v-model="form.front" rows="3" placeholder="请输入问题" class="w-full px-3 py-2 border border-border rounded-sm text-sm focus:outline-none focus:border-primary-500 hover:border-primary-300 font-mono focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--kb-ring)] focus-visible:ring-offset-2 transition-colors"></textarea>
          </div>
          <div>
            <label class="block text-sm font-medium text-foreground mb-1.5">背面（答案）</label>
            <textarea v-model="form.back" rows="3" placeholder="请输入答案" class="w-full px-3 py-2 border border-border rounded-sm text-sm focus:outline-none focus:border-primary-500 hover:border-primary-300 font-mono focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--kb-ring)] focus-visible:ring-offset-2 transition-colors"></textarea>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select v-model.number="form.difficulty" class="w-full px-3 py-2 border border-border rounded-sm text-sm focus:outline-none focus:border-primary-500 hover:border-primary-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--kb-ring)] focus-visible:ring-offset-2 transition-colors">
    ...
    >
      <div class="bg-card rounded-xl w-full max-w-md shadow-xl animate-dropdown">
        <div class="flex items-center justify-between px-6 py-4 border-b border-border">
          <h3 class="text-lg font-semibold text-foreground">AI 生成知识卡片</h3>
        ...
        <div class="px-6 py-4 space-y-4">
          ...
          <div>
            ...
            <select
              ...
              class="w-full px-3 py-2 border border-border rounded-lg text-sm focus:outline-none focus:border-primary-500 hover:border-primary-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--kb-ring)] focus-visible:ring-offset-2 transition-colors"
          ...
          <div>
            <label class="block text-sm font-medium text-foreground mb-1.5">生成数量（3-30）</label>
            <input
