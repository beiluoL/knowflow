...
public class LearningController {
    ...
    public Result<List<LearningPathVO>> paths() {
        // 列表可匿名访问；已登录则带出报名状态与学习进度
        return Result.success(learningService.getPathList(SecurityUtils.getCurrentUserIdNullable()));
    }

    @Operation(summary = "学习路径详情")
    @GetMapping("/paths/{id}")
    public Result<LearningPathVO> pathDetail(@PathVariable Long id, Authentication authentication) {
        Long userId = (Long) authentication.getPrincipal();
        return Result.success(learningService.getPathDetail(id, userId));
    }
    ...
    @Operation(summary = "章节依赖关系图(DAG)")
    @GetMapping("/paths/{pathId}/dag")
    public Result<ChapterDagVO> chapterDag(@PathVariable Long pathId, Authentication authentication) {
        Long userId = (Long) authentication.getPrincipal();
        return Result.success(learningService.getChapterDag(pathId, userId));
    }
    ...
    @Operation(summary = "上报章节视频观看进度（L-FORM-01），返回当前累计进度")
    @PostMapping("/chapters/{id}/video-progress")
    public Result<java.math.BigDecimal> updateVideoProgress(@PathVariable Long id,
                                                            @RequestParam java.math.BigDecimal progress,
                                                            Authentication authentication) {
        Long userId = (Long) authentication.getPrincipal();
        return Result.success(learningService.updateVideoProgress(id, userId, progress));
    }
    ...
    @Operation(summary = "我的数字证书列表")
    @GetMapping("/certificates")
    public Result<List<LearningCertificateVO>> certificates(Authentication authentication) {
        Long userId = (Long) authentication.getPrincipal();
        return Result.success(learningService.listMyCertificates(userId));
    }

    @Operation(summary = "数字证书详情")
    @GetMapping("/certificates/{id}")
    public Result<LearningCertificateVO> certificateDetail(@PathVariable Long id, Authentication authentication) {
        Long userId = (Long) authentication.getPrincipal();
        return Result.success(learningService.getCertificateDetail(id, userId));
    }

    @Operation(summary = "验证数字证书（按验证码核验真伪，可匿名）")
    @GetMapping("/certificates/verify")
    public Result<LearningCertificateVO> verifyCertificate(@RequestParam String certNo) {
        return Result.success(learningService.verifyCertificate(certNo));
    }
}
