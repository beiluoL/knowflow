...
### 2.7 2026-08-15 补漏（`views/CodeAgent.vue` 3 处漏网死按钮）
> §2.2（2026-08-09）已将 CodeAgent 整体标合规，但复盘扫描发现模型卡片内 3 个可点击 `div`（`model-icon` / `model-info` / `model-card-params`，均 `@click.stop="openParamsModal(m)"`）未转可键盘访问。本轮补齐：三处均加 `role="button"` + `tabindex="0"` + `@keydown.enter.prevent.self="($event.target as HTMLElement).click()"` + 焦点环 + `transition-colors`。
